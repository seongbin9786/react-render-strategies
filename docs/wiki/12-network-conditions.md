...
1. **세로로**: slow3g 열에서 CSR과 스트리밍 SSR의 격차가 최대다. as-is/to-be 데모는 이 열에서 봐야 극적이다.
2. **가로로**: wifi 행에서는 모든 전략이 대동소이하다. **"내 맥북 + localhost에서 차이가 안 보인다"는 반론이 무의미한 이유**다 — [14. 측정 방법론](./14-measurement-methodology.md).
