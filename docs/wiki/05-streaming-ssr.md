...
> **한 줄 요약**: 페이지를 Suspense 경계로 섹션 단위로 쪼개면 준비된 셸을 즉시 보내고 느린 섹션은 같은 HTTP 응답 스트림으로 뒤늦게 흘려보낼 수 있다 — "가장 느린 데이터가 페이지 전체를 볼모로 잡는" 블로킹 SSR의 문제를 푼다. 대가는 경계 배치가 곧 UX 설계가 되고(잘못 쪼개면 화면이 팝콘처럼 튄다), 중간 인프라가 버퍼링하면 이득이 통째로 사라지며, 응답을 시작한 뒤에는 HTTP 상태 코드를 바꿀 수 없다(200 고정)는 것이다.
>
> **선행 문서**: [03. SSR](./03-ssr.md)

## Suspense와 fallback — 이 문서의 전제

Suspense는 "아직 준비 안 된 자식" 대신 대체 UI(**fallback**)를 보여주는 React 컴포넌트다. `<Suspense fallback={<스켈레톤/>}>느린섹션</Suspense>`처럼 트리를 감싸 **경계(boundary)**를 만들고, 경계 안 컴포넌트가 데이터 대기 등으로 아직 렌더를 끝낼 수 없는 상태(**suspend**)면 React가 그 자리에 fallback을 대신 그린다. 서버 렌더에서 이 경계는 HTML의 절단선이 된다 — 경계 밖 트리(**셸**)는 데이터를 기다릴 것 없이 즉시 보내고, 경계 안은 준비되는 대로 같은 응답 스트림에 뒤이어 보낸다. [03](./03-ssr.md)과 [07](./07-hydration.md)이 "05 참고"로 미뤄 둔 Suspense 경계의 정의가 바로 이것이다.

## 동작 원리

렌더러(`renderToPipeableStream`)는 트리를 렌더하다 경계 안 컴포넌트가 데이터를 기다리며 suspend하면, 그 서브트리를 멈추고 fallback을 HTML에 대신 써 넣은 뒤 나머지 렌더를 계속한다 — 그래서 셸은 어떤 데이터도 기다릴 필요 없이 완성 즉시 flush된다. 멈췄던 서브트리는 서버에서 계속 렌더되어, 완성되는 대로 같은 스트림의 추가 청크로 실린다.
...
그래서 셸은 **어떤 Suspense 경계도 기다리지 않고** 먼저 flush된다 — "빠른" 섹션의 콘텐츠도 별도의 후속 청크로 온다. (셸이 준비된 시점에 이미 resolve가 끝난 — 데이터가 도착해 렌더까지 끝난 — 경계가 있다면 그 콘텐츠는 첫 청크에 포함될 수도 있다.)
