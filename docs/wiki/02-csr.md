...
첫 콘텐츠는 HTML→JS→데이터 세 번의 왕복이 전부 끝나야 나온다 — 화면을 만드는 모든 일을 브라우저가 하므로, 서버가 해 줄 수 있는 것은 파일 서빙뿐이기 때문이다.
...
첫 화면 콘텐츠가 덜 중요하고 세션이 길수록 CSR이 유리하다 — 왕복 3회의 초기 비용은 한 번만 내면 되고, 이후 내비게이션은 이미 받은 JS가 API 호출만으로 처리하므로 세션이 길수록 그 비용이 상각되기 때문이다.
...
반대로 CSR의 비용은 전부 첫 로드에 몰려 있으므로, 첫 화면 자체가 목적이거나 그 비용을 감당하기 어려운 쪽(느린 회선·저사양 기기·JS를 실행하지 않는 크롤러)이 방문자일수록 불리하다.

- **첫 방문 콘텐츠가 중요한 페이지**: 랜딩, 상품 목록, 글. FCP/LCP가 번들+데이터에 볼모로 잡힌다.
- **느린 회선·저사양 기기**: JS 다운로드와 평가 비용을 사용자 기기가 전부 낸다.
- **SEO / 링크 미리보기**: 링크 미리보기 봇과 상당수 크롤러는 JS를 실행하지 않거나 실행을 한참 뒤로 미룬다 — 그들에게는 왕복 1에서 받은 빈 HTML이 페이지의 전부이므로, CSR 페이지는 제목도 본문도 없는 문서로 읽힌다.
